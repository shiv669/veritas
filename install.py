...
def download_model(model_name=None):
    ...
    try:
        # We'll use the Hugging Face hub to download the model
...
def download_research_templates():
...
def main():
    ...
    if not args.skip_dependencies and not install_dependencies():
        return 1
    ...
    if not download_research_templates():
        logger.warning("⚠️ Failed to set up research templates")
        # But continue with installation
    ...
    if success:
        ...
        logger.error("❌ Installation completed with some errors")
        return 1
