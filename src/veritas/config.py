...
class Config:
    ...
    MODELS_DIR: ClassVar[str] = os.path.join(BASE_DIR, "models")
    LOGS_DIR: ClassVar[str] = os.path.join(BASE_DIR, "logs")
    SCRIPTS_DIR: ClassVar[str] = os.path.join(BASE_DIR, "scripts")
    TESTS_DIR: ClassVar[str] = os.path.join(BASE_DIR, "tests")
    DOCS_DIR: ClassVar[str] = os.path.join(BASE_DIR, "docs")
    
    # Input/output directories
    INPUT_DIR: ClassVar[str] = os.path.join(DATA_DIR, "input")
    OUTPUT_DIR: ClassVar[str] = os.path.join(DATA_DIR, "output")
    CHUNKS_DIR: ClassVar[str] = os.path.join(DATA_DIR, "chunks")
    INDICES_DIR: ClassVar[str] = os.path.join(MODELS_DIR, "faiss")
    
    # Temporary directory for files - Uses SSD by default
    TEMP_DIR: ClassVar[str] = "/Volumes/8SSD/veritas/tmp"
    
    # Model settings
    EMBEDDING_MODEL: ClassVar[str] = "sentence-transformers/all-MiniLM-L6-v2"  # Converts text to vectors
    LLM_MODEL: ClassVar[str] = os.path.join(MODELS_DIR, "mistral-7b")  # The AI that generates answers
    
    # Chunking settings
    DEFAULT_CHUNK_SIZE: ClassVar[int] = 512  # How many words in each text chunk
    DEFAULT_CHUNK_OVERLAP: ClassVar[int] = 128  # How much chunks overlap to avoid splitting ideas
    
    # Retrieval settings
    TOP_K: ClassVar[int] = 5  # How many relevant chunks to retrieve for each question
    SIMILARITY_THRESHOLD: ClassVar[float] = 0.6  # How similar a chunk must be to be considered relevant
    
    # M4 Mac performance settings
    CPU_THREADS: ClassVar[int] = 4
    MEMORY_LIMIT_GB: ClassVar[int] = 100  # Max memory limit in GB
    ...
    @classmethod
    def setup_environment(cls) -> Dict[str, str]:
        """
